...
export async function readSkillResource(skillName: string, relativePath: string): Promise<{
  skillName: string;
  relativePath: string;
  absolutePath: string;
  content: string;
...
async function findSkillByName(skillName: string): Promise<SkillSummary | null> {
...
function toSkillSummary(skillDir: string, skillFilePath: string, markdown: string): SkillSummary {
...
async function walk(rootDir: string, currentDir: string, collected: string[]): Promise<void> {
  const entries = await readdir(currentDir, { withFileTypes: true });

  for (const entry of entries) {
    if (collected.length >= MAX_RESOURCE_COUNT) return;
