...
export function listChatSessions(workspaceDir: string): ChatSessionSummary[] {
...
export function createChatSession(request: CreateChatSessionRequest): ChatSessionSummary {
  const db = getDatabase();
...
export function getChatSessionMessages(sessionId: string): ChatHistoryMessage[] {
  const db = getDatabase();
  const statement = db.prepare(`
    SELECT
      id,
...
function getSessionSummaryById(sessionId: string): ChatSessionSummary | null {
  const db = getDatabase();
  const row = db
    .prepare(`
      SELECT
...
function mapSessionRow(row: Record<string, unknown>): ChatSessionSummary {
  return {
    id: String(row.id),
...
function parseToolCalls(raw: unknown): PersistedToolCall[] | undefined {
  ...
  try {
    ...
    return Array.isArray(parsed) ? (parsed.filter(isPersistedToolCall) as PersistedToolCall[]) : undefined;
  } catch {
    return undefined;
  }
}
...
function asMessageRole(value: unknown): ChatHistoryMessage["role"] {
  switch (value) {
    case "user":
    case "assistant":
    case "tool":
